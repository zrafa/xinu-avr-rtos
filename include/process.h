...
#define	PNMLEN		8+1		/* Length of process "name"			*/
...
#define	isbadpid(x)	( ((pid16)(x) < 0) || \
			  ((pid16)(x) >= NPROC) || \
			  (proctab[(x)].prstate == PR_FREE))


/* Definition of the process table (multiple of 16 bits) */
...
struct procent {			/* Entry in the process table		*/
	...
	uint8	*prstkbase;		/* Base of run time stack			*/
	uint16	prstklen;		/* Stack length in bytes			*/
	char	prname[PNMLEN];	/* Process name						*/
	sid16	prsem;			/* Semaphore on which process waits	*/
	pid16	prparent;		/* ID of the creating process		*/
	umsg16	prmsg;			/* Message sent to this process		*/
	bool8	prhasmsg;		/* Nonzero iff msg is valid			*/
};

/* Marker for the top of a process stack (used to help detect overflow)	*/
#define	STACKMAGIC	0xAA

extern	struct	procent proctab[];
extern	int16	prcount;	/* Currently active processes		*/
extern	pid16	currpid;	/* Currently executing process		*/
